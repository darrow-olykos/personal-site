...
const Todos = (0, import_index_f6c97f68.c)(($$result, $$props, $$bindings, slots) => {
  let { todos } = $$props;
...
<div class="${"todos svelte-dmxqmd"}"><h1>Todos</h1>
	...
	${(0, import_index_f6c97f68.b)(todos, (todo) => {
    return `<div class="${["todo svelte-dmxqmd", todo.done ? "done" : ""].join(" ").trim()}"><form action="${"/todos/" + (0, import_index_f6c97f68.e)(todo.uid) + ".json?_method=PATCH"}" method="${"post"}"><input type="${"hidden"}" name="${"done"}"${(0, import_index_f6c97f68.j)("value", todo.done ? "" : "true", 0)} class="${"svelte-dmxqmd"}">
				<button class="${"toggle svelte-dmxqmd"}" aria-label="${"Mark todo as " + (0, import_index_f6c97f68.e)(todo.done ? "not done" : "done")}"></button></form>

			<form class="${"text svelte-dmxqmd"}" action="${"/todos/" + (0, import_index_f6c97f68.e)(todo.uid) + ".json?_method=PATCH"}" method="${"post"}"><input aria-label="${"Edit todo"}" type="${"text"}" name="${"text"}"${(0, import_index_f6c97f68.j)("value", todo.text, 0)} class="${"svelte-dmxqmd"}">
				<button class="${"save svelte-dmxqmd"}" aria-label="${"Save todo"}"></button></form>

			<form action="${"/todos/" + (0, import_index_f6c97f68.e)(todo.uid) + ".json?_method=DELETE"}" method="${"post"}"><button class="${"delete svelte-dmxqmd"}" aria-label="${"Delete todo"}" ${todo.pending_delete ? "disabled" : ""}></button></form>
		</div>`;
  })}
</div>`;
});
